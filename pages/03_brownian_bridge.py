...
FIX 3: SE Reduction now uses grouped bars instead of overlayed bars,
with a small gap between groups.

FIX 4: MSE decomposition explicitly shows the grey grid on both
subplot panels.
"""
...
def run_sim(
    ...
    naive_p, naive_se = mc_naive_sweep(
        opt,
        S0,
        K,
        H,
...
def update_graphs(data):
    ...
    ref_half = (
        np.exp(icn)
        * np.array(STEP_GRID) ** (-0.5)
    )

    ref_one = (
        np.exp(icn)
        * np.array(STEP_GRID) ** (-1.0)
    )
    ...
    f2.update_layout(
        **base,
        title=dict(
            text="Log-log bias comparison",
            font=dict(size=12)
        )
    )

    # Explicit tick values to prevent Plotly from
    # abbreviating 20 -> 2 and 500 -> 5.
    f2.update_xaxes(
        title_text="N",
        type="log",
        tickvals=STEP_GRID,
        ticktext=[str(n) for n in STEP_GRID]
    )
    ...
    # IMPORTANT:
    # grouped bars instead of overlayed bars
    f3.update_layout(
        **base,
        ...
        title=dict(
            text="SE Reduction  (Rao-Blackwell)",
            font=dict(size=12)
        )
    )
    ...
    # IMPORTANT:
    # Explicit grey grid on BOTH subplot panels.
    f4.update_xaxes(
        title_text="N",
        type="category",
