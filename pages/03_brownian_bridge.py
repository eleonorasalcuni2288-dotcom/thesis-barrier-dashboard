...
def mc_naive_sweep(opt, S0, K, H, r, q, sigma, T, steps, M=15000):
    """
    Computes naive discrete-monitoring Monte Carlo option prices.
    Ignores inter-node barrier crossings, yielding O(dt^0.5) discretization bias.
    """
    prices, ses = [], []
    ...
    for N in steps:
        ...
        d = np.exp(-r * T) * pay
        prices.append(d.mean())
        ses.append(d.std(ddof=1) / np.sqrt(M))
...
def mc_bb_sweep(opt, S0, K, H, r, q, sigma, T, steps, M=15000):
    """
    Computes Monte Carlo option prices using Brownian Bridge inter-node corrections.
    Calculates exact analytical crossing probability between discrete sampling points.
    Applies Rao-Blackwellization to achieve variance reduction and O(dt) bias convergence.
    """
    prices, ses = [], []
    ...
    for N in steps:
        ...
        # Prepend initial log-price matrix S0
        lnSf = np.hstack([np.full((M, 1), np.log(S0)), lnS])
        a = lnSf[:, :-1]
        b = lnSf[:, 1:]

        # Analytical probability of breaching lnH within sub-interval (dt)
        pn = np.where(
            (a > lnH) & (b > lnH),
            np.exp(-2.0 * (a - lnH) * (b - lnH) / var_dt),
            1.0
        )
        pn = np.clip(pn, 0.0, 1.0)

        # Discrete path validity check
        ns = np.all(lnS > lnH, axis=1)

        # Survival probability over continuous path via conditional expectation
        sp = np.where(ns, np.prod(1.0 - pn, axis=1), 0.0)
        ST = np.exp(lnS[:, -1])

        if opt == "do":
            pay = np.maximum(K - ST, 0) * sp
        else:
            pay = np.maximum(ST - K, 0) * (1.0 - sp)
    ...
    children=[
        ...
        html.Div(
            [
                html.H1(
                    ...
                    style={"fontSize": "20px", "fontWeight": "600", "margin": "0 0 3px", "color": DARK}
                ),
                html.P(
                    "Analytical inter-node correction — bias O(Δt), variance reduction (Rao-Blackwell)",
        ...
        html.Div(
            ...
            children=[
                # Sidebar Controls
                html.Div(
                    style={**CARD, "width": "240px", "flexShrink": "0"},
                    children=[
                        html.P(
                            "PARAMETERS",
                            style={"fontSize": "10px", "letterSpacing": "0.12em", "color": GRAY, "marginBottom": "16px"}
                        ),
                        *[
                            html.Div(
                                [
                        ...
                        html.P(
                            "OPTION TYPE",
                            style={"fontSize": "10px", "letterSpacing": "0.12em", "color": GRAY, "marginBottom": "10px"}
                        ),
                        dcc.RadioItems(
                            id=f"{PID}opt-type2",
                            options=[
                                {"label": " Down-and-Out Put", "value": "do"},
                ...
                # Graphs Grid
                html.Div(
                    style={"flex": "1", "display": "flex", "flexDirection": "column", "gap": "16px", "minWidth": "480px"},
                    children=[
                        # Row 1
                        html.Div(
                            ...
                            children=[
                                html.Div(
                                    ...
                                    children=[
                                        dcc.Graph(id=f"{PID}conv2", style={"height": "300px"}, config={"displayModeBar": False})
                                    ]
                                ),
                                html.Div(
                                    style={**CARD, "flex": "1", "minWidth": "300px"},
                                    children=[
                                        dcc.Graph(id=f"{PID}loglog2", style={"height": "300px"}, config={"displayModeBar": False})
                                    ]
                                ),
                            ]
                        ),
                        ...
                        html.Div(
                            ...
                            children=[
                                html.Div(
                                    style={**CARD, "flex": "1", "minWidth": "300px"},
                                    children=[
                                        dcc.Graph(id=f"{PID}se2", style={"height": "280px"}, config={"displayModeBar": False})
                                    ]
                                ),
                                html.Div(
                                    style={**CARD, "flex": "1", "minWidth": "300px"},
                                    children=[
                                        dcc.Graph(id=f"{PID}mse2", style={"height": "280px"}, config={"displayModeBar": False})
                                    ]
                                ),
                            ]
                        ),
        ...
        html.P(
            "S₀=100, K=100, M=15,000 paths. Press 'Run Simulation' to update.",
...
@callback(
    Output(f"{PID}h2-val", "children"),
    Output(f"{PID}sigma2-val", "children"),
    Output(f"{PID}t2-val", "children"),
    Output(f"{PID}r2-val", "children"),
    Input(f"{PID}h2", "value"),
    Input(f"{PID}sigma2", "value"),
    Input(f"{PID}t2", "value"),
    Input(f"{PID}r2", "value"),
)
...
@callback(
    ...
    State(f"{PID}h2", "value"),
    State(f"{PID}sigma2", "value"),
    State(f"{PID}t2", "value"),
    State(f"{PID}r2", "value"),
...
def run_sim(n_clicks, h, sigma_pct, t_sc, r_pct, opt):
        ...
        [
            ...
            *[
                html.Div(
                    [
                        html.Span(lbl, style=LABEL),
                        html.Span(val, style={**VALUE, "color": col})
                    ],
    ...
    store = {
        "naive_p": naive_p.tolist(),
        "naive_se": naive_se.tolist(),
        "bb_p": bb_p.tolist(),
        "bb_se": bb_se.tolist(),
        "bs_val": bs_val,
    }
...
@callback(
    Output(f"{PID}conv2", "figure"),
    Output(f"{PID}loglog2", "figure"),
    Output(f"{PID}se2", "figure"),
    Output(f"{PID}mse2", "figure"),
...
def update_graphs(data):
    ...
    naive_p = np.array(data["naive_p"])
    naive_se = np.array(data["naive_se"])
    bb_p = np.array(data["bb_p"])
    ...
    f1 = go.Figure()
    f1.add_hline(
        y=bs_val, line_color=DARK, line_dash="dash", line_width=2,
        annotation_text="BS analytical", annotation_font_size=10
    )
    f1.add_trace(go.Scatter(
        x=STEP_GRID, y=naive_p, mode="lines+markers", name="MC Naive",
    ...
    f1.add_trace(go.Scatter(
        x=STEP_GRID, y=bb_p, mode="lines+markers", name="MC BB",
    ...
    # 2. Log-Log Bias Convergence
    nb = np.abs(naive_p - bs_val) + 1e-8
    bb_b = np.abs(bb_p - bs_val) + 1e-8
    log_N = np.log(STEP_GRID)

    sn, icn = np.polyfit(log_N, np.log(nb), 1)
    sb, icb = np.polyfit(log_N, np.log(bb_b), 1)
    ...
    f2.add_trace(go.Scatter(
        x=STEP_GRID, y=nb, mode="lines+markers", name=f"Naive (slope={sn:.2f})",
        line=dict(color=FUCHSIA, width=2.5), marker=dict(size=7, symbol="square")
    ))
    f2.add_trace(go.Scatter(
        x=STEP_GRID, y=bb_b, mode="lines+markers", name=f"BB (slope={sb:.2f})",
        line=dict(color=BLUE, width=2.5), marker=dict(size=7)
    ))
    f2.add_trace(go.Scatter(
        x=STEP_GRID, y=ref_half, mode="lines", name="O(N⁻⁰·⁵)",
        line=dict(color=FUCHSIA, width=1, dash="dot"), opacity=0.6
    ))
    f2.add_trace(go.Scatter(
        x=STEP_GRID, y=ref_one, mode="lines", name="O(N⁻¹·⁰)",
    ...
    f2.update_xaxes(title_text="N", type="log", tickvals=STEP_GRID, ticktext=[str(n) for n in STEP_GRID])
